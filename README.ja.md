...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 084 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

完全描写は都市や場面を細部で埋め、太い黒輪郭やコンピュータフィルターは空気、光、奥行き、独立出版物の抑制を失わせます。

**Panel 084** は細い濃色の幾何線で主体、建築、道路、空間関係を抽出し、density-based stippling で空、地面、遠景、影、光の空気層を作ります。複雑な背景を削り、明確な焦点、大きな余白、少数の元写真由来の差し色だけを残します。

### 向いている用途

- 都市、建築、道路、空間関係を細い幾何線と遠近誘導線で明確にしたい場合
- 塗り込まず、点描密度で空気、光、被写界深度を表したい場合
- ブルーグレー、オフホワイト、暖白を中心に、元写真由来の少数の spot color だけを使いたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 084 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** ミニマル都市線描 · 幾何学骨格 · 密度ベースの点描 · 透視リーディングライン · 限定色 · 詩的余白
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 都市随筆、建築スケッチ、場面構造図のような現代的線描ポスター |
| **識別しやすい特徴** | 細い幾何線、明確な leading lines、点描密度勾配、ブルーグレーと暖白、少量の spot color |
| **入力の尊重** | 主体、輪郭、構造、姿勢、空間誘導を残し、色は元写真の最も気質ある部分だけから取る |
...
```text
主体・構造・姿勢・空間誘導を抽出 → 細い幾何骨格へ減らす → 遠近線と leading lines を作る → 点描密度で空気・光・奥行き・影を形成 → ブルーグレーと暖白を組む → 元写真由来の少数 spot color を加える → 余白に静かな編集注記を置く
```

## 完成作の特徴

- 縦線、横線、折線、曲線、遠近線で空間骨格を作る。
- 太い陰影ではなく点の密度で空気、光、被写界深度、空間階層を作る。
- 道路、建築、電線、樹木、境界が一つの焦点へ leading lines を作る。
- 複雑な背景を少数の構造手掛かりと大きな呼吸空間へ減らす。
- ブルーグレーと暖白を主とし、暖黄や元色は抑制された差し色に限る。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-084-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 084 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-084 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-084 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-084 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-084 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 084 を適用し、共通設定は一度だけ解決
/xxd-panel-084 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-084`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-084-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-084-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

84 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 084 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–084） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
