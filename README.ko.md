...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 084이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

완전 묘사는 도시와 장면을 세부로 채우고 굵은 검은 윤곽과 컴퓨터 필터는 공기, 빛, 깊이, 독립 출판의 절제를 없앱니다.

**Panel 084**는 가는 짙은 기하 선으로 피사체, 건축, 도로, 공간 관계를 추리고 density-based stippling으로 하늘, 땅, 원경, 그림자, 빛의 공기층을 만듭니다. 복잡한 배경을 지우고 명확한 초점, 넓은 여백, 소수의 원본 유래 강조색만 남깁니다.

### 이런 경우에 적합합니다

- 도시, 건축, 도로, 공간 관계를 가는 기하 선과 원근 유도선으로 명확히 하고 싶을 때
- 채워 넣지 않고 점묘 밀도로 공기, 빛, 피사계 심도를 표현하고 싶을 때
- 청회색, 미색, 따뜻한 흰색 중심에 원본 유래 spot color만 소량 쓰고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 084로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 미니멀 도시 선묘 · 기하 골격 · 밀도 기반 점묘 · 원근 리딩 라인 · 제한 색상 · 시적 여백
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 도시 수필, 건축 스케치, 장면 구조도 같은 현대적 선화 포스터 |
| **대표 특징** | 가는 기하 선, 명확한 leading lines, 점묘 밀도 그라데이션, 청회색과 따뜻한 흰 바탕, 소량 spot color |
| **입력 존중 방식** | 피사체, 윤곽, 구조, 자세, 공간 유도를 남기고 원본의 가장 기질 있는 색만 사용함 |
...
```text
피사체·구조·자세·공간 유도 추출 → 가는 기하 골격으로 축약 → 원근선과 leading lines 구성 → 점묘 밀도로 공기·빛·깊이·그림자 형성 → 청회색과 따뜻한 흰 바탕 조직 → 원본 유래 소수 spot color 추가 → 여백에 조용한 편집 주석 배치
```

## 완성작의 특징

- 세로선, 가로선, 꺾은선, 곡선, 원근선으로 공간 골격을 만듭니다.
- 무거운 음영 대신 점 밀도로 공기, 빛, 피사계 심도, 공간 층위를 만듭니다.
- 도로, 건축, 전선, 나무, 경계가 하나의 초점으로 leading lines를 만듭니다.
- 복잡한 배경을 소수 구조 단서와 넓은 호흡 공간으로 줄입니다.
- 청회색과 따뜻한 흰색이 중심이고 따뜻한 노랑이나 원본 색은 절제된 강조에 한정합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-084-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 084의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-084 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-084 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-084 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-084 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-084 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-084 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 084 적용, 공통 설정은 한 번만 확정
/xxd-panel-084 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-084`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-084-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-084-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

84개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 084까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–084) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
