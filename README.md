<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 084 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 084 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
提取主体、轮廓、结构、姿态与空间关系 → 删除复杂背景 → 用竖横折弧线和透视线建立几何骨架 → 以道路、建筑、电线、灯具或树木形成视觉引导 → 用点描疏密塑造空气、光感与景深 → 从原图重组蓝灰、米白、浅灰、暖白和少量强调色 → 让留白与轻量编辑注记完成城市诗意
```

## 成品中最容易识别的风格特点

- 线条可有轻微手绘抖动，但保持细、克制、结构清楚，不使用粗黑轮廓。
- 点描分布于天空、地面、远景、树影、灯光或主体周围，以密度建立空间而非堆积纹理。
- 主体不必居中，但必须有清晰视觉焦点和自然的方向引导。
- 色彩以蓝灰、米白、浅灰、暖白等低刺激综合色为主，仅以少量暖黄、金色或原图精神色点亮节奏。
- 文字细瘦、安静、略带印刷误差，可沿留白、主体边缘或引导线自然排布。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-084-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–084） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
